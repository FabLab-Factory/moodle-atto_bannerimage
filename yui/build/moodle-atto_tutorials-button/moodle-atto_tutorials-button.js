...
var COMPONENTNAME = 'atto_bannerimage';
var LOGNAME = 'atto_bannerimage';


var SELECTORS = {
  URLINPUT: '.atto_bannerimage_urlentry'
};

var CSS = {
  INPUTSUBMIT: 'atto_bannerimage_selectcolumns',
  NEWWINDOW: 'atto_bannerimage_openinnewwindow',
  URLINPUT: 'atto_bannerimage_urlentry',
};
...
Y.namespace('M.atto_bannerimage').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
  ...
  initializer: function() {
        ...
        if(contains(this.get('enabled_templates'),t)) {
          TEMPLATE += '<a class="tutorialstemplateicon {{CSS.INPUTSUBMIT}}" alt="' + templates[t].title +  '" title="' +
                      templates[t].title + '" data-template="'+ t + '">' +
                          '<img src="'+ M.util.image_url("ed/" + templates[t].icon,"atto_bannerimage") + '"/>' +
                      '</a>';
        }
      }
    ...
    Y.Array.each(editor_icons, function(theicon) {
      ...
      this.addButton({
        icon: 'ed/' + theicon,
        iconComponent: 'atto_bannerimage',
        buttonName: theicon,
